...
class RLFB(nn.Module):
    ...
    def __init__(
        self, in_channels, mid_channels=None, out_channels=None, esa_channels=16
    ):
...
class RLFB2(nn.Module):
    """
    Residual Local Feature Block (RLFB2).
    """
...
class Generator(nn.Module):
    def __init__(self, cfg):
        ...
        upscale = cfg.scale
        in_channels = cfg.num_in_ch
        out_channels = cfg.num_out_ch
        feature_channels = cfg.num_feat
        mid_channels = cfg.mid_feat

        self.conv_1 = conv_layer(in_channels, feature_channels, kernel_size=3)

        self.block_1 = RLFB(feature_channels, mid_channels)
        self.block_2 = RLFB2(feature_channels, mid_channels)
        self.block_3 = RLFB2(feature_channels, mid_channels)
        self.block_4 = RLFB(feature_channels, mid_channels)

        self.conv_2 = conv_layer(
            feature_channels, feature_channels, kernel_size=3
        )
    ...
    def forward(self, x):
        ...
        out_b2 = self.block_2(out_b1 + out_feature)
        out_b3 = self.block_3(out_b2 + out_feature)
        out_b4 = self.block_4(out_b3 + out_feature)
