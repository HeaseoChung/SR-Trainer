...
def get_model_flops(
    ...
    assert (
        type(input_res) is tuple
    ), "Please provide the size of the input image."
...
def get_model_activation(model, input_res, input_constructor=None):
    ...
    (
        activation_count,
        num_conv,
    ) = activation_model.compute_average_activation_cost()
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        ...
        if isinstance(module, (nn.Conv2d, nn.Conv3d, nn.ConvTranspose2d)):
            ...
            module, (nn.ReLU, nn.PReLU, nn.ELU, nn.LeakyReLU, nn.ReLU6)
        ):
