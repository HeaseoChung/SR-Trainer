...
class SSIM:
    ...
    def create_window(self):
        torch.Tensor(
            [
                exp(
                    -((x - self.window_size // 2) ** 2)
                    / float(2 * self.sigma**2)
                )
...
class LPIPS:
    ...
    def __call__(self, img1, img2):
        return 1 - self.lpips(img1, img2).squeeze()
...
class ERQA:
    ...
    def __call__(self, img1, img2):
        img1 = img1.squeeze()
        img2 = img2.squeeze()
        img1 = (torch.transpose(img1, 0, 2).data.cpu().numpy() * 255.0).astype(
            np.uint8
        )
        img2 = (torch.transpose(img2, 0, 2).data.cpu().numpy() * 255.0).astype(
            np.uint8
        )
