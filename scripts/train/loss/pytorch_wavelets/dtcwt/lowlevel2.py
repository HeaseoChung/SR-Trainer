...
class DTCWTForward2(nn.Module):
    ...
    def __init__(
        self, biort="farras", qshift="qshift_a", J=3, mode="symmetric"
    ):
        super().__init__()
        self.biort = biort
        self.qshift = qshift
...
def prep_filt_quad_afb2d(h0a, h1a, h0b, h1b, device=None):
    ...
    cols = torch.tensor(
        np.copy(cols), dtype=torch.get_default_dtype(), device=device
    )
    rows = torch.tensor(
        np.copy(rows), dtype=torch.get_default_dtype(), device=device
    )
    return cols, rows
...
def icplxdual2D(
    yl, yh, level1="farras", qshift="qshift_a", mode="periodization"
):
    # Get the filters
