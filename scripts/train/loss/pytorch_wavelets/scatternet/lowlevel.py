...
class ScatLayerj2_f(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx, x, h0o, h1o, h0a, h0b, h1a, h1b, mode, bias, combine_colour
    ):
        #  bias = 1e-2
        #  bias = 0
        ctx.in_shape = x.shape
        ...
        if combine_colour:
            ...
            s0, reals, imags = fwd_j2plus(
                s0, h0a, h1a, h0b, h1b, False, 1, mode
            )
            s1_j2 = torch.sqrt(
                reals[:, :, 0] ** 2
                + imags[:, :, 0] ** 2
            ...
            if x.requires_grad:
                ...
                z = x.new_zeros(1)
                ctx.save_for_backward(
                    h0o, h1o, h0a, h0b, h1a, h1b, z, z, z, z, z, z
                )
        ...
        if ctx.needs_input_grad[0]:
            ...
            if ctx.combine_colour:
                ...
                ds1_j1 = (
                    1
                    / 4
                    * F.interpolate(ds1_j1, scale_factor=2, mode="nearest")
                )
                q = ds2_j1.shape
    ...
    def forward(
        ...
        if combine_colour:
            ...
            s1_j1, reals, imags = fwd_j1_rot(
                s1_j1, h0o, h1o, h2o, False, 1, mode
            )
            s2_j1 = torch.sqrt(reals**2 + imags**2 + bias**2)
            if x.requires_grad:
                dsdx2_1 = reals / s2_j1
